...
export default function BlogsListPage() {
  ...
  const filteredBlogs = blogs
    ? blogs.filter((blog) =>
        [blog.title, blog.content, blog.author.first_name, blog.author.last_name, blog.author.email]
          .filter(Boolean)
          .some((field) => (field as string).toLowerCase().includes(searchTerm.toLowerCase()))
      )
    : [];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBlogs.map((blog) => (
            <div key={blog.id} className="relative p-4 border rounded-lg shadow bg-white dark:bg-gray-900">
              ...
              <p className="text-gray-600 dark:text-gray-300">By {blog.author.first_name || blog.author.email}</p>
